...
class GAT(torch.nn.Module):
    def __init__(self, dataset, out_channels, n_hidden_layers):
        ...
        self.conv1 = GATConv(dataset.num_node_features, out_channels, edge_dim=dataset.num_edge_features)
        self.n_hidden_layers = n_hidden_layers
        self.hidden_conv = []
        for i in range(self.n_hidden_layers):
            self.hidden_conv.append(GATConv(out_channels, out_channels, edge_dim=dataset.num_edge_features))
